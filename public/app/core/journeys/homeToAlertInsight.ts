 ...
 *
 * Start triggers:
 *   - grafana_homepage_alerts_card_clicked — one of four card actions. `action` selects the
 *     destination and therefore which end event completes the journey; `msSinceLoad` carries the
 *     card-visible → click dwell so the click event's own timing is queryable on the journey.
 *
...
registerJourneyTriggers('home_to_alert_insight', (tracker) => {
  return onInteraction('grafana_homepage_alerts_card_clicked', (props) => {
